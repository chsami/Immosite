...
$( document ).ready(function() {
    ...
    $('label').on('click', function () {
        if ($(this).attr('id') == 'id-lbl-huur') {
            ...
            $(this).css('background-color', '#0d3532');
            $('#price-min-id').val(0);
            $('#price-max-id').attr('value', 5000000);
            $( "#price-range-min" ).text(0);
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data != undefined && data.length > 0) {
                        $('#featured_property > .list-unstyled > .item').empty();
    ...
    function loadPandTemplate(referentienummer, type, stad, straat, postcode, status,
        ...
        '<a href="#">' +
        '<img src=' + media_url +
        '' + profiel_foto + ' class="img-responsive" style="height:30%;" alt="a" />' +
        '<label>' + status + '</label>' +
        '<figcaption>' +
    ...
    function preSearch() {
       /* $('input').change(function() {
            ...
            setTimeout(function() {
                console.log("CLICKED IT00");
                formData = $('#normal-search-form').serialize();
                $.ajax({
                    url: "/presearch",
                    type: 'POST',
                    data: formData,
